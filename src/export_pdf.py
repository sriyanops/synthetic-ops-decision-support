...
# ----------------------------
from metrics import compute_metrics, build_data_lineage
...
def apply_kpi_colors(table_obj, metrics: dict):
    ...
    # Row index map in kpi_rows:
    # 0 = header
    # 1 = Avg Package Volume
    # 2 = Capacity Utilization
    # 3 = On-Time Delivery Rate
    # 4 = Avg Daily Exceptions
    # 5 = Labor Efficiency
    # 6 = Avg Cost per Package

    cu = float(metrics["capacity_utilization"])

    # Avg Package Volume (row 1) — color it using utilization status (volume vs capacity)
    if cu < UTIL_WARN_LOW:
        styles.append(("BACKGROUND", (1, 1), (1, 1), GREEN))
    elif cu < UTIL_WARN_HIGH:
        styles.append(("BACKGROUND", (1, 1), (1, 1), YELLOW))
    else:
        styles.append(("BACKGROUND", (1, 1), (1, 1), RED))
...
def scenario_mix_rows(df: pd.DataFrame):
    """
    Builds rows matching your two-table layout:
    [Scenario, Days, Avg Volume, Avg Util, Avg OnTime, Avg Exceptions, Avg LaborEff, Avg Cost]
    """
    df = df.copy()
    df["date"] = pd.to_datetime(df["date"])

    required = {"scenario", "package_volume", "network_capacity", "on_time_rate", "exceptions", "labor_hours", "cost_per_package"}
    missing = [c for c in required if c not in df.columns]
    if missing:
        return []

    df["util"] = df["package_volume"] / df["network_capacity"]
...
def top_drivers_tables(df: pd.DataFrame):
    """
    Returns:
      - worst_on_time (bottom 5 on_time_rate)
      - high_ex (top 5 exceptions)
      - high_util (top 5 utilization)
      - high_cost (top 5 cost_per_package)
    """
    df = df.copy()
    df["date"] = pd.to_datetime(df["date"])
    ...
    high_cost = df.nlargest(5, "cost_per_package")[["date", "scenario", "cost_per_package", "on_time_rate", "package_volume"]]

    return worst_on_time, high_ex, high_util, high_cost
...
def build_charts(df: pd.DataFrame, window_days: int) -> dict:
    ...
    _save_line_chart(d, "package_volume", f"Daily Package Volume (last {window_days} available days)", paths["volume"])
    _save_line_chart(d, "cap_util", f"Capacity Utilization (last {window_days} available days)", paths["util"])
    _save_line_chart(d, "on_time_rate", f"On-Time Rate (last {window_days} available days)", paths["ontime"])
    _save_line_chart(d, "exceptions", f"Daily Exceptions (last {window_days} available days)", paths["exceptions"])
    _save_line_chart(d, "labor_eff", f"Labor Efficiency pkgs/hr (last {window_days} available days)", paths["labor_eff"])
    _save_line_chart(d, "cost_per_package", f"Cost per Package (last {window_days} available days)", paths["cost"])
...
def export():
    if not DATA_PATH.exists():
        raise FileNotFoundError(
            ...
            f"Expected CSV at: data/sample/shipments_sample.csv"
        )
    ...
    # Window sizing
    chart_window = min(60, days) if days > 0 else 0

    # Option B: pull from modules
    metrics = compute_metrics(df)
    recommendations = generate_recommendations(metrics)

    # Local helpers (still deterministic + auditable)
    worst_on_time, high_ex, high_util, high_cost = top_drivers_tables(df)
    mix_rows = scenario_mix_rows(df)
    chart_paths = build_charts(df, chart_window) if chart_window > 0 else {}
    ...
    if EXEC_SUMMARY:
        ...
        story.append(
            Paragraph(
                f"Current performance snapshot (recent 14 days): Capacity utilization = {fmt_pct(metrics['capacity_utilization'])}; "
        ...
        story.append(
            Paragraph(
                "What’s driving risk: Disruption scenarios typically drive service degradation; confirm using the Top Drivers tables and KPI trend charts.",
                body,
            )
        )
        story.append(
    ...
    arch = [
        ...
        ["1) Data Layer", "Input dataset: data/sample/shipments_sample.csv (Synthetic dataset structured like an ops feed)"],
        ...
        ["5) Outputs", "reports/FedEx_Project_Ops_Report.pdf + charts in reports/charts/"],
    ]
    ...
    flow_text = (
    "data/sample/shipments_sample.csv<br/>"
    "↓<br/>"
    ...
    "↓<br/>"
    "reports/FedEx_Project_Ops_Report.pdf"
)
    ...
    ctx = [
        ["Field", "Value"],
        ["Data Source", "data/sample/shipments_sample.csv"],
        ["Days Analyzed", str(days)],
        ["Date Range", f"{date_min} → {date_max}"],
        ["Trend Window Used for Charts", f"Last {chart_window} available days"],
    ]
    story.append(make_table(ctx, W, col_fracs=[0.34, 0.66], wrap_cells=True))
    ...
    if mix_rows:
        mix1 = [["Scenario", "Days", "Avg Volume", "Avg Util"]] + [[r[0], r[1], r[2], r[3]] for r in mix_rows]
        mix2 = [["Scenario", "Avg On-Time", "Avg Exceptions", "Avg Labor Eff (pkgs/hr)", "Avg Cost"]] + [
            [r[0], r[4], r[5], r[6], r[7]] for r in mix_rows
        ]
        ...
        story.append(Paragraph("Scenario mix unavailable (missing 'scenario' column).", body))
    ...
    # Page 4 — Charts
    story.append(PageBreak())
    story.append(Paragraph(f"KPI Trends (last {chart_window} available days)", h_style))
    ...
    story.append(Spacer(1, 10))
    story.append(
        Paragraph("Highest Cost per Package Days (top 5)", ParagraphStyle("Sub4", parent=styles["Heading3"], fontSize=10))
    )
    c = [["Date", "Scenario", "Cost/Package", "On-Time", "Volume"]]
    for _, r in high_cost.iterrows():
        c.append(
            [
                str(pd.to_datetime(r["date"]).date()),
                _display_scenario(r["scenario"]),
                f"${float(r['cost_per_package']):.2f}",
                fmt_pct(r["on_time_rate"]),
                fmt_int(r["package_volume"]),
            ]
        )
    story.append(make_table(c, W, col_fracs=[0.24, 0.20, 0.18, 0.16, 0.22], wrap_cells=False))
